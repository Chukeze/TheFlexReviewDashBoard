...
export async function GET(req: NextRequest) {
  ...
  const ratingMin = sp.get('ratingMin') ? parseFloat(sp.get('ratingMin') as string) : undefined;
  const ratingMax = sp.get('ratingMax') ? parseFloat(sp.get('ratingMax') as string) : undefined;
  const category = sp.get('category');
  const from = sp.get('from');
  const to = sp.get('to');

  const file = path.join(process.cwd(), 'data', 'mock-hostaway-reviews.json');
  const raw = JSON.parse(await fs.readFile(file, 'utf-8')) as { status: string; result: RawReview[] };
  let reviews = normalizeMany(raw.result);

  // filters
  if (channel) reviews = reviews.filter(r => r.channel.toLowerCase() === channel.toLowerCase());
  if (listingId) reviews = reviews.filter(r => r.listingId === listingId);
  if (ratingMin !== undefined) reviews = reviews.filter(r => r.overall >= ratingMin);
  if (ratingMax !== undefined) reviews = reviews.filter(r => r.overall <= ratingMax);
  if (category) reviews = reviews.filter(r => Object.prototype.hasOwnProperty.call(r.categories, category));

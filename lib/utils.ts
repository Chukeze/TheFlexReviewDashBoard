...
export function normalizeHostawayReviews(raws: any[]): Review[] {
  return raws.map((raw) => {
    const listingId = raw.listingId || 'unknown-listing-id'
    const slug = slugify(raw.listingName || 'unknown-listing')
...
export const daysSince = (iso: string) =>
  Math.round((now() - new Date(iso).getTime()) / MS_DAY)
export const lastNDays = (r: Review[], d: number) =>
  r.filter((x) => now() - new Date(x.submittedAt).getTime() <= d * MS_DAY)
...
export function matchesDetailedFilters(
  r: Review,
  {
    channel,
    listingId,
  ...
  if (Number.isFinite(minRating) && r.overall < minRating) return false
  if (Number.isFinite(maxRating) && r.overall > maxRating) return false
  if (category && !(r.categories && category in r.categories)) return false

  // Reuse your existing time logic
  if (
    !inCurrentWindow(
      r.submittedAt,
      hasCustomRange,

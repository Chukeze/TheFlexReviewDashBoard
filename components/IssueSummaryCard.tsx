...
import FollowUpBadges from './FollowUpBadge'
import FollowUpButton from './FollowUpButton'
import { TrendCells } from './TrendCells'
import { CANON_RULES, DROP_GENERIC } from '@/lib/constants'
...
function extractTopIssues(
  ...
  const POSITIVE_FILTERS = new Set([
    'good',
    'great',
    'nice',
    'love',
  ...
  for (const r of low) {
    ...
    const uniq = Array.from(new Set(condenseTokens(toks)))
      .filter((t) => {
        if (POSITIVE_FILTERS.has(t)) return false // skip positive noise
        const isBigram = t.includes(' ')
        const hasIssueWord = t
          .split(' ')
          .some((w) => ISSUE_LEXICON.has(normalizeWord(w)))
        const looksNeg = NEGATORS.test(raw)
        return (
          (isBigram && hasIssueWord) ||
          ISSUE_LEXICON.has(normalizeWord(t)) ||
          !(STOPWORDS.has(t) || /^\d+$/.test(t))
        )
      })
  ...
  const issues = useMemo(
    () =>
      extractTopIssues(
        ...
        from,
        to,
        activeChannel || onlyChannel || undefined
      ),
    [reviews, windowDays, from, to, activeChannel, onlyChannel]
  )
  return (
    <div
      className="card"
      role="region"
      aria-labelledby="issue-summary-card"
      id="issue-summary-card"
      style={{ minWidth: 0 }}
    >
      <div
        className="row"
      ...
      >
        <h3 style={{ marginTop: 0 }} id="issue-summary-card-title">
      ...
      {!issues.length ? (
        ...
        <table className="table" style={{ tableLayout: 'fixed' }}>
          <caption className="sr-only">
            Most frequent low‑rating review issues
          </caption>
          <thead>
            <tr>
              <th style={{ width: '28%' }}>Issue</th>
              <th style={{ marginRight: '20.65rem' }}>Mentions</th>
              <th>Listings</th>
              <th>Top Listing</th>
              ...
              <tr key={row.term}>
                ...
                <td>
                  <FollowUpButton
                    term={row.term}
                    listingId={row.topListingId}
                    listingName={row.topListingName}
                  />
                </td>
              </tr>
            ))}
